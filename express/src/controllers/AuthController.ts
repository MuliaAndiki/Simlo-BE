...
class AuthController {
  ...
  public loginDeveloper = async (req: Request, res: Response) => {
    try {
      const service = await AuthService.LoginDeveloperService(res, req);
  ...
  public logout = [
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        const service = await AuthService.LogoutService(res, users.id);
        if (!service) {
          res.status(400).json({
            status: 400,
            message: "bad request",
  ...
  public GetMe = [
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        const service = await AuthService.GetMeService(res, users.id);
